...
async function injectContentScript(tabId: number): Promise<void> {
	if (browser.scripting) {
		console.log('[Kiipu Clipper] Using scripting API');
		await browser.scripting.executeScript({
			target: { tabId },
			files: ['content.js']
		});
	} else {
		console.log('[Kiipu Clipper] Using tabs.executeScript fallback');
		await browser.tabs.executeScript(tabId, { file: 'content.js' });
	}
	console.log('[Kiipu Clipper] Injection completed, waiting for init...');
	...
	console.log('[Kiipu Clipper] Post-injection ping succeeded');
}
...
async function ensureContentScriptLoadedInBackground(tabId: number): Promise<void> {
	try {
		...
		await browser.tabs.sendMessage(tabId, { action: "ping" });
		console.log('[Kiipu Clipper] Content script ping succeeded');
	} catch (error) {
		...
		console.log('[Kiipu Clipper] Ping failed, injecting content script...', error);
		...
		if (typedRequest.action === "forceInjectContentScript") {
			...
			if (tabId) {
				injectContentScript(tabId)
					.then(() => sendResponse({ success: true }))
					.catch((error) => {
						console.error('[Kiipu Clipper] forceInjectContentScript failed:', error);
						sendResponse({ success: false, error: error instanceof Error ? error.message : String(error) });
					});
				return true;
			} else {
		...
		if (typedRequest.action === "sendMessageToTab") {
			...
			if (tabId && message) {
				...
				ensureContentScriptLoadedInBackground(tabId).then(() => {
					console.log('[Kiipu Clipper] Sending message to tab:', message.action);
					return browser.tabs.sendMessage(tabId, message);
				}).then((response) => {
					console.log('[Kiipu Clipper] Tab response:', response ? 'has content=' + !!((response as any).content) : response);
					sendResponse(response);
				}).catch((error) => {
					console.error('[Kiipu Clipper] Error sending message to tab:', error);
					sendResponse({
						success: false,
						error: error instanceof Error ? error.message : String(error)
					});
				...
				browser.tabs.query({active: true, currentWindow: true}).then((tabs) => {
					...
					if (currentTab && currentTab.id) {
						browser.tabs.update(currentTab.id, { url: url }).then(() => {
							...
							console.error('Error opening Kiipu URL:', error);
							sendResponse({
								success: false,
								error: error instanceof Error ? error.message : String(error)
							});
